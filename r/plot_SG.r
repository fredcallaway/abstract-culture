...
source("base.r")
FIGS_PATH <- "figs/model-SG/"

fixed_points <- read_csv("../results/SG_fixed_points.csv")
...
asymptote_plot <- fixed_points %>% 
    ...
    no_gridlines +
    facet_wrap(~D, nrow=1) +
    labs(fill="asymptotic compositionality")

figure("asymptote", asymptote_plot)

# %% --------
figure("tmp", fixed_points %>% 
    mutate(imbalance = pmax(S / G, G / S), total = S * G) %>% 
    ggplot(aes(imbalance, asymptote, color=total)) +
    geom_point() +
    # geom_line() +
    facet_wrap(~D)
)
...
figure("evolution", evolution_plot)

# %% --------

phase <- read_csv("../results/SG_phase.csv")
