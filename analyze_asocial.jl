...
participants = load_participants(versions...; all_generations=true)
...
R"""
tdf %>%
    ggplot(aes(trial_number, factor(pid), color=factor(path_length))) +
    geom_point() +
    scale_colour_manual(values=c(
        BLACK, RED
    ), aesthetics=c("fill", "colour"), name="path length") +
    ylab("Participant")
...
tdf %>%
    ...
    mutate(compositionality = path_length == 2) %>%
    ggplot(aes(trial_number, 1*compositionality)) +
    geom_smooth()
...
R"""
tdf %>%
    ggplot(aes(trial_number, duration, group=factor(pid))) +
    # geom_point() +
    geom_smooth(se=F) +
    # facet_wrap(~pid) +
    ylim(0, 100)
